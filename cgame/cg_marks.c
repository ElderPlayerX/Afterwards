...
void CG_ImpactMark( qhandle_t markShader, const vec3_t origin, const vec3_t dir, 
	...
	mp = cg_activeMarkPolys.prevMark;
	for ( ; mp != &cg_activeMarkPolys; mp = mp->prevMark ) {
		VectorSubtract( mp->origin, origin, delta );
		if ( radius <= mp->radius + 4 && VectorLength( delta ) < ( radius + mp->radius ) * MIN_MARK_DISTANCE ) {
			// don't draw the new one because there's already an old one
			return;
		}
	}	
