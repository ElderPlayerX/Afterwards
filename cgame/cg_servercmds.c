...
static void CG_ParseTeamInfo( void ) {
	...
	for ( i = 0 ; i < numSortedTeamPlayers ; i++ ) {
		...
		cgs.clientinfo[ client ].location = atoi( CG_Argv( i * 4 + 3 ) );
		cgs.clientinfo[ client ].health = atoi( CG_Argv( i * 4 + 4 ) );
		cgs.clientinfo[ client ].curWeapon = atoi( CG_Argv( i * 4 + 5 ) );
	}
}
...
void CG_ParseServerinfo( void ) {
	...
	cgs.delagWeapons = atoi( Info_ValueForKey( info, "g_delagWeapons" ) );
...
void CG_AutoJoin( void ) {
	...
	if ( !strcmp( CG_Argv( 1 ), "r" ) ) {
		// join tribes
		CG_AwInitMenu( SM_MODELS_RED );
	} else if ( !strcmp( CG_Argv( 1 ), "b" ) ) {
		// join australians
		CG_AwInitMenu( SM_MODELS_BLUE );
	}
}
