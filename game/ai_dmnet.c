...
int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal) {
	if (goal->flags & GFL_ITEM) {
		...
		if (trap_BotTouchingGoal(bs->origin, goal)) {
			...
			// throw away old weapon to take the new one
			ge = &g_entities[goal->entitynum];
			botent = &g_entities[bs->entitynum];
		
			if ( ge->item && ge->item->giType == IT_WEAPON ) {
//				G_Printf( "Reached weapon: %s\n", ge->item->pickup_name );
	
				if ( ge->item->giTag != bs->cur_ps.weapons[ weLi[ge->item->giTag].category ] ) {
					// throw it away if it's a different gun
					DropWeapon( botent, weLi[ge->item->giTag].category, qtrue );
				}
			}
...
int AINode_Stand(bot_state_t *bs) {
	...
	if (bs->lastframe_health > bs->inventory[INV_HEALTH]) {
...
/*void BotClearPath(bot_state_t *bs, bot_moveresult_t *moveresult) {
	...
	if (bs->kamikazebody) {
		...
		if ( !(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)) ) {
			...
			if (moveresult->weapon) {
				...
				if (bs->cur_ps.wpcat == moveresult->weapon) {
	...
	if (bs->blockedbyavoidspot_time > FloatTime() &&
		...
		if (bestmine != -1) {
			...
			if (bs->inventory[INVENTORY_DBT] > 0 && bs->inventory[INVENTORY_DBT_AMMO] > 0)
				moveresult->weapon = CATINDEX_DBT;
			else if (bs->inventory[INVENTORY_GRENADE] > 0 && bs->inventory[INVENTORY_GRENADE_AMMO] > 0)
				moveresult->weapon = CATINDEX_GRENADE;
			else {
				moveresult->weapon = 0;
			}
			if (moveresult->weapon) {
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
	...
	if (bs->activatestack->shoot) {
		...
		if (bsptrace.fraction >= 1.0 || bsptrace.ent == goal->entitynum) {
			targetvisible = qtrue;
			// if holding the right weapon
			if (bs->cur_ps.wpcat == bs->activatestack->weapon) {
