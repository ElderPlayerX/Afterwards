...
int BotCTFCarryingFlag(bot_state_t *bs) {
	if (qtrue) return CTF_FLAG_NONE; // gametype != GT_CTF
...
==================
*/
void BotCheckWeapon(bot_state_t *bs) {
	// reset wrong weapon selection
	if ( !bs->cur_ps.weapons[bs->wpCatNum] || bs->wpCatNum < 0 || bs->wpCatNum >= CT_NUM_CATEGORIES ) bs->wpCatNum = 0;

	// select weapon
	trap_EA_SelectWeapon(bs->client, bs->wpCatNum);
}
...
void BotChooseWeapon(bot_state_t *bs) {
	...
	// check for best fighting weapon
	if (bs->cur_ps.weaponstate != WEAPON_RAISING && bs->cur_ps.weaponstate != WEAPON_DROPPING) {
		newWeaponCat = weLi[ trap_BotChooseBestFightWeapon(bs->ws, bs->inventory) ].category;

		if (bs->wpCatNum != newWeaponCat) {
			bs->weaponchange_time = FloatTime();
			bs->wpCatNum = newWeaponCat;
//			BotAI_Print(PRT_MESSAGE, "bs->wpCatNum = %d\n", bs->wpCatNum);
		}
	}
	...
	// reload the current gun if no enemy is in sight and few ammo is left
	if ( bs->enemy < 0 && ( bs->cur_ps.clipammo[ bs->cur_ps.wpcat ] < 0.5f * weLi[ bs->cur_ps.weapons[bs->cur_ps.wpcat] ].clip_max || 
		 ( bs->noenemy_time + 5 < FloatTime() && bs->cur_ps.clipammo[ bs->cur_ps.wpcat ] < weLi[ bs->cur_ps.weapons[bs->cur_ps.wpcat] ].clip_max ) ) ) {
...
void BotUpdateInventory(bot_state_t *bs) {
	...
	// weapons
	for ( i = 0; i < CT_NUM_CATEGORIES; i++ ) {
		// by weapon
		if ( bs->cur_ps.weapons[i] ) {
			bs->inventory[INV_WEAPON_TABLE + bs->cur_ps.weapons[i]] = qtrue;
			bs->inventory[INV_AMMO_TABLE + bs->cur_ps.weapons[i]] = bs->cur_ps.clipammo[i] + bs->cur_ps.bagammo[i];
			bs->inventory[INV_CLIPAMMO_TABLE + bs->cur_ps.weapons[i]] = bs->cur_ps.clipammo[i];
		}
		// by category
		bs->inventory[INV_MELEE + i] = bs->cur_ps.weapons[i];
		bs->inventory[INV_MELEE_AMMO + i] = bs->cur_ps.clipammo[i] + bs->cur_ps.bagammo[i];
	}

	// powerups
	bs->inventory[INV_GAMETYPE] = gametype;
	bs->inventory[INV_HEALTH] = bs->cur_ps.stats[STAT_HEALTH];
	// insert goal here!
...
/*
==================
BotResetBattleInventory
==================
*/
void BotResetBattleInventory(bot_state_t *bs) {
...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INV_HEALTH] < 40) return 0;
	//attack
	return 100;
	/*
	//if the bot can use the rocket launcher
	if (bs->inventory[INVENTORY_DBT] > 0 &&
			bs->inventory[INVENTORY_DBT_AMMO] > 1) return 100;
	//if the bot can use the flamethrower
	if (bs->inventory[INVENTORY_FLAME] > 0 &&
			bs->inventory[INVENTORY_FLAME_AMMO] > 3) return 95;
	//if the bot can use the grenade launcher
	if (bs->inventory[INVENTORY_GRENADE] > 0 &&
			bs->inventory[INVENTORY_GRENADE_AMMO] > 3) return 90;
	//if the bot can use the assault rifle
	if (bs->inventory[INVENTORY_A_RIFLE] > 0 &&
			bs->inventory[INVENTORY_A_RIFLE_AMMO] > 10) return 85;
	//if the bot can use the minigun
	if (bs->inventory[INVENTORY_MINIGUN] > 0 &&
			bs->inventory[INVENTORY_MINIGUN_AMMO] > 30) return 80;
	//if the bot can use the shotgun
	if (bs->inventory[INVENTORY_ROTSHOTGUN] > 0 &&
			bs->inventory[INVENTORY_ROTSHOTGUN_AMMO] > 2) return 75;
	//if the bot can use the micro
	if (bs->inventory[INVENTORY_MICRO] > 0 &&
			bs->inventory[INVENTORY_MICRO_AMMO] > 40) return 70;
	//if the bot can use the sniper
	if (bs->inventory[INVENTORY_SNIPER] > 0 &&
			bs->inventory[INVENTORY_SNIPER_AMMO] > 1) return 65;
	//if the bot can use the handgun
	if (bs->inventory[INVENTORY_HANDGUN] > 0 &&
			bs->inventory[INVENTORY_HANDGUN_AMMO] > 4) return 60;
	//if the bot can use the standard
	if (bs->inventory[INVENTORY_STANDARD] > 0 &&
			bs->inventory[INVENTORY_STANDARD_AMMO] > 1) return 50;
...
float BotFeelingBad(bot_state_t *bs) {
	if (bs->inventory[INV_HEALTH] < 30) {
		return 100;
	}
	if (bs->inventory[INV_HEALTH] < 50) {
		return 80;
	}
	return 0;
}
...
int BotHasPersistantPowerupAndWeapon(bot_state_t *bs) {
	...
	//if the bot can use the rocket launcher
	if (bs->inventory[INVENTORY_DBT] > 0 &&
			bs->inventory[INVENTORY_DBT_AMMO] > 1) return qtrue;
	//if the bot can use the flamethrower
	if (bs->inventory[INVENTORY_FLAME] > 0 &&
			bs->inventory[INVENTORY_FLAME_AMMO] > 3) return qtrue;
	//if the bot can use the grenade launcher
	if (bs->inventory[INVENTORY_GRENADE] > 0 &&
			bs->inventory[INVENTORY_GRENADE_AMMO] > 3) return qtrue;
	//if the bot can use the assault rifle
	if (bs->inventory[INVENTORY_A_RIFLE] > 0 &&
			bs->inventory[INVENTORY_A_RIFLE_AMMO] > 10) return qtrue;
	//if the bot can use the minigun
	if (bs->inventory[INVENTORY_MINIGUN] > 0 &&
			bs->inventory[INVENTORY_MINIGUN_AMMO] > 30) return qtrue;
	//if the bot can use the shotgun
	if (bs->inventory[INVENTORY_ROTSHOTGUN] > 0 &&
			bs->inventory[INVENTORY_ROTSHOTGUN_AMMO] > 2) return qtrue;
	//if the bot can use the micro
	if (bs->inventory[INVENTORY_MICRO] > 0 &&
			bs->inventory[INVENTORY_MICRO_AMMO] > 40) return qtrue;
	//if the bot can use the sniper
	if (bs->inventory[INVENTORY_SNIPER] > 0 &&
			bs->inventory[INVENTORY_SNIPER_AMMO] > 1) return qtrue;
	//if the bot can use the handgun
	if (bs->inventory[INVENTORY_HANDGUN] > 0 &&
			bs->inventory[INVENTORY_HANDGUN_AMMO] > 4) return qtrue;
	//if the bot can use the standard
	if (bs->inventory[INVENTORY_STANDARD] > 0 &&
			bs->inventory[INVENTORY_STANDARD_AMMO] > 1) return qtrue;
	//otherwise the bot is not feeling too good
	return qfalse;*/
}
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	healthdecrease = bs->lasthealth > bs->inventory[INV_HEALTH];
	//remember the current health value
	bs->lasthealth = bs->inventory[INV_HEALTH];
	//setup noenemy_time
	if ( bs->enemy < 0 && !bs->noenemy_time ) {
		bs->noenemy_time = FloatTime();
	}
	//
	if (curenemy >= 0) {
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	trap_BotGetWeaponInfo(bs->ws, bs->cur_ps.weapons[ bs->wpCatNum ], &wi);
	//get the weapon specific aim accuracy and or aim skill
	if (wi.number == WP_STANDARD || wi.number == WP_HANDGUN) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_PISTOLS, 0, 1);
	}
	else if (wi.number == WP_MICRO || wi.number == WP_SUB) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SMGS, 0, 1);
	}
	else if (wi.number == WP_SHOTGUN || wi.number == WP_A_RIFLE || wi.number == WP_TRIPACT || wi.number == WP_DBT) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_RIFLES, 0, 1);
	}
	else if (wi.number == WP_SNIPER || wi.number == WP_PHOENIX || wi.number == WP_OMEGA) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SNIPER, 0, 1);
	}
	else if (wi.number == WP_MINIGUN || wi.number == WP_FLAMETHROWER || wi.number == WP_RPG || wi.number == WP_THERMO) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_BIGGUNS, 0, 1);
	}
	else if (wi.number == WP_GRENADE_LAUNCHER || wi.number == WP_FRAG_GRENADE || wi.number == WP_FIRE_GRENADE) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADES, 0, 1);
	}

	if (wi.number == WP_TRIPACT || wi.number == WP_DBT || wi.number == WP_FLAMETHROWER ||wi.number == WP_RPG) {
		aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_PROJECTILES, 0, 1);
	} else if (wi.number == WP_GRENADE_LAUNCHER || wi.number == WP_FRAG_GRENADE || wi.number == WP_FIRE_GRENADE) {
		aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_GRENADES, 0, 1);
	}
...
void BotCheckAttack(bot_state_t *bs) {
	...
	//hold fire with Minigun
	if ( bs->cur_ps.weapons[ bs->wpCatNum ] == WP_MINIGUN ) firethrottle = 1.0;
	...
	// reload the current gun if necessary
	if ( bs->cur_ps.clipammo[ bs->cur_ps.wpcat ] == 0 ) {
		trap_EA_Action(bs->client, ACTION_RELOAD);
		return;
	}
...
==================
*/
void AI_BuyWeapon( bot_state_t *bs ) {
	int sidearm = 0;
	int	primary = 0;
	int try;
	int rank;

	rank = BG_GetTacticalRank(bs->cur_ps.persistant[PERS_SCORE]);

	if ( rank > 0 ) {
		// try to find a sidearm
		while ( !sidearm ) {
			try = (int)(random() * ( WP_NUM_WEAPONS - 1 ) ) + 1;
			if ( weLi[try].category == CT_SIDEARM && rank >= weLi[try].min_rank && 
				1 / (float)(rank - weLi[try].min_rank+1) > random() ) 
				sidearm = try;
		}
		// try to find a primary weapon
		while ( !primary ) {
			try = (int)(random() * ( WP_NUM_WEAPONS - 1 ) ) + 1;
			if ( weLi[try].category == CT_PRIMARY && rank >= weLi[try].min_rank && 
				1 / (float)(rank - weLi[try].min_rank+1) > random() ) 
				primary = try;
		}
		// buy chosen weapons
		trap_EA_Command(bs->client, va("buy %s", weLi[sidearm].name) );
		trap_EA_Command(bs->client, va("buy %s", weLi[primary].name) );

		// G_Printf(va("Bot has chosen: %s %s\n", weLi[sidearm].name, weLi[primary].name));
	} else {
		// just buy a standard pistol (loly bugfix)
		trap_EA_Command(bs->client, va("buy %s", weLi[WP_STANDARD].name) );
	}
}
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (bs->setupcount > 0) {
		...
		//
		bs->lastframe_health = bs->inventory[INV_HEALTH];
	...
	if (!BotIntermission(bs)) {
		...
		//reset battle inventory info
		BotResetBattleInventory(bs);
	...
	// buy weapons if not yet done in GT_TACTICAL
	if ( gametype == GT_TACTICAL && g_entities[bs->client].client->buyzone_time > 0 &&
		!(g_entities[bs->client].client->buy_flags & BF_PRIMARY) ) {
		AI_BuyWeapon( bs );
	}
	...
	// choose the best weapon every AI frame
	BotChooseWeapon(bs);
}
