...
//	qboolean won;
	...
	if ( level.clients[playerClientNum].sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		Com_sprintf( msg, sizeof(msg), "postgame %i %i %i", level.numNonSpectatorClients, playerClientNum, accuracy );
	}
...
static gentity_t *SpawnModelOnVictoryPad( gentity_t *pad, vec3_t offset, gentity_t *ent, int place ) {
	...
	if( body->s.weapon == WP_NONE ) { // Herby modify this !
		body->s.weapon = WP_STANDARD;
	}
	if( qfalse ) { // Herby modify this ! (Gauntlet)
