	...
	for ( n = 0; n < g_numBots ; n++ ) {
		...
		if (i >= g_maxclients.integer) {
			...
			if (num <= 0) {
				...
				if (team == TEAM_RED) teamstr = "Tribes";
				else if (team == TEAM_BLUE) teamstr = "Australians";
				else teamstr = "";
...
qboolean G_BotConnect( int clientNum, qboolean restart ) {
	...
	if (!BotAISetupClient( clientNum, &settings, restart ) || g_gametype.integer < GT_DM ) {
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	// not in GT_DM (no bot support)
	if ( g_gametype.integer < GT_DM ) {
		G_Printf( S_COLOR_RED "Bots are currently only supported in Deathmatch.\n" );
		return;
	}
...
static void G_LoadBotsFromFile( char *filename ) {
	...
	// very first token needs to be an "afterwards"
	bufp = buf;
	token = COM_Parse( &bufp );
	if ( !strcmp( token, "afterwards" ) ) {
		g_numBots += G_ParseInfos( bufp, MAX_BOTS - g_numBots, &g_botInfos[g_numBots] );
	}
}
