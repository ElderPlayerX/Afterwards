...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( ent->moverState == MOVER_POS2 ) {
		if ( ent->wait >= 0 ) {
			...
			ent->think = ReturnToPos1;
			ent->nextthink = level.time + 50; // 50 msec later
		}
		return;
	}
...
================
*/
static void SpectatorTeleport( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	int i, axis;
	...
	if (fabs(other->s.origin[axis] - ent->r.absmax[axis]) <
		...
		origin[axis] = ent->r.absmin[axis] - 16;
		dir[axis] = -1;
	}
	else {
		origin[axis] = ent->r.absmax[axis] + 16;
		dir[axis] = 1;
	}
	for (i = 0; i < 3; i++) {
		if (i == axis) continue;
...
void Touch_DoorTrigger( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	if ( other->client && other->client->sess.sessionTeam < TEAM_SPECTATOR &&
		ent->parent->moverState != MOVER_1TO2 && ent->parent->moverState != ROTATOR_1TO2 ) {
