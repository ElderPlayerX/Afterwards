...
static void UI_CDKeyMenu_DrawKey( void *self ) {
	...
	if( val == 1 ) {
		UI_DrawProportionalString( 320, 376, "Please enter your CD Key", UI_CENTER|UI_SMALLFONT|UI_CURSORSHADOW, color_yellow );
	}
	else if ( val == 0 ) {
		UI_DrawProportionalString( 320, 376, "The CD Key appears to be valid, thank you", UI_CENTER|UI_SMALLFONT|UI_CURSORSHADOW, color_white );
	}
	else {
		UI_DrawProportionalString( 320, 376, "The CD Key is not valid", UI_CENTER|UI_SMALLFONT|UI_CURSORSHADOW, color_red );
	}
}
...
static void UI_CDKeyMenu_Init( void ) {
	...
	cdkeyMenuInfo.menu.showlogo = LOGO_AFTERWARDS;
