...
static void Demos_MenuEvent( void *ptr, int event ) {
	...
	case ID_GO:
		UI_ForceMenuOff ();
		trap_Cmd_ExecuteText( EXEC_APPEND, va( "demo %s.dm_67\n", s_demos.list.itemnames[s_demos.list.curvalue] ) );
		break;
...
static void Demos_MenuInit( void ) {
	...
	s_demos.menu.showlogo = LOGO_AFTERWARDS;
	...
	s_demos.list.numitems			= trap_FS_GetFileList( "demos", "dm_67", s_demos.names, NAMEBUFSIZE );
	...
	for ( i = 0; i < s_demos.list.numitems; i++ ) {
		...
		// strip extension
		len = strlen( demoname );
		if (!Q_stricmp(demoname +  len - 6,".DM_67"))
			demoname[len-6] = '\0';
