...
void InGame_Event( void *ptr, int notification ) {
	...
	case ID_TEAM:
		// open the Afterwards menu stored in cgame if in GT_TACTICAL...
		trap_GetConfigString(CS_SERVERINFO, info, MAX_INFO_STRING);   

		if ( atoi( Info_ValueForKey( info,"g_gametype" ) ) == GT_TACTICAL ) {
			UI_PopMenu();
			trap_Cvar_Set("cg_awMenu", "1");
		} else UI_TeamMainMenu();
		break;
	...
	case ID_SERVERINFO:
		if ( !trap_Cvar_VariableValue( "sv_running" ) ) {
			trap_Cvar_VariableStringBuffer( "cl_currentServerAddress", adrstr, sizeof(adrstr) );
			UI_ServerInfoMenu( adrstr );
		} else {
			UI_ServerInfoMenu( "localhost" );
		}
		break;
...
void InGame_MenuInit( void ) {
	...
	s_ingame.removebots.color				= cg_color;
	...
	s_ingame.server.color				= cg_color;
	...
	s_ingame.restart.color				= cg_color;
	...
	s_ingame.resume.color					= cg_color;
