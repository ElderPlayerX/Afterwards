...
static void PlayerModel_UpdateModel( void )
{
	...
	UI_PlayerInfo_SetInfo( &s_playermodel.playerinfo, LEGS_IDLE, TORSO_STAND, viewangles, moveangles, WP_SUB, qfalse );
}
...
static void PlayerModel_PicEvent( void* ptr, int event )
{
	...
	buffptr  = s_playermodel.modelnames[modelnum] + strlen("models/aw_players/");
...
static void PlayerModel_BuildList( void )
{
	...
	numdirs = trap_FS_GetFileList("models/aw_players", "/", dirlist, 2048 );
	dirptr  = dirlist;
	for (i=0; i<numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		numfiles = trap_FS_GetFileList( va("models/aw_players/%s",dirptr), "tga", filelist, 2048 );
		fileptr  = filelist;
		for (j=0; j<numfiles && s_playermodel.nummodels < MAX_PLAYERMODELS;j++,fileptr+=filelen+1)
		{
			...
			if (!Q_stricmpn(skinname,"icon_",5))
			{
				Com_sprintf( s_playermodel.modelnames[s_playermodel.nummodels++],
					...
					"models/aw_players/%s/%s", dirptr, skinname );
...
static void PlayerModel_SetMenuItems( void )
{
	...
	{
		// strip icon_
		buffptr  = s_playermodel.modelnames[i] + strlen("models/aw_players/");
		pdest    = strstr(buffptr,"icon_");
		if (pdest)
		{
...
static void PlayerModel_MenuInit( void )
{
	...
	s_playermodel.menu.showlogo = LOGO_AFTERWARDS;
