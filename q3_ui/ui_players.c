...
/*	if ( weaponNum == WP_SUB || weaponNum == WP_A_RIFLE || weaponNum == WP_ROCKET_LAUNCHER ) {
...
static qboolean UI_RegisterClientSkin( playerInfo_t *pi, const char *modelName, const char *skinName ) {
	...
	Com_sprintf( filename, sizeof( filename ), "models/aw_players/%s/lower_%s.skin", modelName, skinName );
	pi->legsSkin = trap_R_RegisterSkin( filename );

	Com_sprintf( filename, sizeof( filename ), "models/aw_players/%s/upper_%s.skin", modelName, skinName );
	pi->torsoSkin = trap_R_RegisterSkin( filename );

	Com_sprintf( filename, sizeof( filename ), "models/aw_players/%s/head_%s.skin", modelName, skinName );
...
qboolean UI_RegisterClientModelname( playerInfo_t *pi, const char *modelSkinName ) {
	...
	Com_sprintf( filename, sizeof( filename ), "models/aw_players/%s/head.md3", modelName );
...
void UI_PlayerInfo_SetInfo( playerInfo_t *pi, int legsAnim, int torsoAnim, vec3_t viewAngles, vec3_t moveAngles, weapon_t weaponNumber, qboolean chat ) {
	...
	// torso animation
	if ( torsoAnim == TORSO_STAND || torsoAnim == TORSO_STAND2 || torsoAnim == TORSO_STAND_PIST || 
		torsoAnim == TORSO_STAND_UP || torsoAnim == TORSO_STAND_ROCK || torsoAnim == TORSO_STAND_GREN ) {
		torsoAnim = weAnims[weLi[weaponNum].weapon_type].stand;
	}

	if ( torsoAnim == TORSO_ATTACK || torsoAnim == TORSO_ATTACK2 || torsoAnim == TORSO_ATTACK_PIST || 
		torsoAnim == TORSO_ATTACK_UP || torsoAnim == TORSO_ATTACK_ROCK || torsoAnim == TORSO_ATTACK_GREN ) {
		torsoAnim = weAnims[weLi[weaponNum].weapon_type].attack;
