...
static void PlayerSettings_DrawPlayer( void *self ) {
	...
	if ( strcmp( buf, s_playersettings.playerModel ) != 0 ) {
		...
		viewangles[PITCH] = 0;
		viewangles[ROLL]  = 0;
		UI_PlayerInfo_SetInfo( &s_playersettings.playerinfo, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin, WP_SUB, qfalse );
	}
...
static void PlayerSettings_MenuEvent( void* ptr, int event ) {
	...
	case ID_HANDICAP:
		trap_Cvar_Set( "handicap", va( "%i", 100 - 10 * s_playersettings.handicap.curvalue ) );
		break;
