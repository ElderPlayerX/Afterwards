...
vec4_t menu_text_color	    = {1.00f, 1.00f, 1.00f, 1.00f};
vec4_t menu_dim_color       = {0.00f, 0.00f, 0.00f, 0.75f};
...
vec4_t color_orange	        = {1.00f, 0.53f, 0.00f, 1.00f};
...
vec4_t text_color_normal	= {1.00f, 0.53f, 0.00f, 1.00f};	// light orange
vec4_t text_color_highlight = {1.00f, 0.83f, 0.00f, 1.00f};	// bright yellow
	...
	else
	{
		...
		{	
			if (b->focuscolor)			
			{
				...
				color = pulse_color;
			color[3] = 0.5+0.5*sin(uis.realtime/(float)PULSE_DIVISOR);
...
static void SpinControl_Draw( menulist_s *s )
{
	...
	if ( s->generic.flags & QMF_CURSORSHADOW ) {
		style |= UI_CURSORSHADOW;
	}
...
void ScrollList_Draw( menulist_s *l )
{
	...
	for( column = 0; column < l->columns; column++ ) {
		...
		for( i = base; i < base + l->height; i++) {
			...
			if ( l->generic.flags & QMF_CURSORSHADOW ) {
				style |= UI_CURSORSHADOW;
			}
...
void Menu_Cache( void )
{
	...
	uis.charsetProp		= trap_R_RegisterShaderNoMip( "menu/art/awfont1.tga" );
	uis.charsetPropB	= trap_R_RegisterShaderNoMip( "menu/art/font2_prop.tga" );
	uis.fontGlow		= trap_R_RegisterShaderNoMip( "menu/art/font_glow.tga" );
	uis.fontGlowEnd		= trap_R_RegisterShaderNoMip( "menu/art/font_glow_end.tga" );
	uis.cursor          = trap_R_RegisterShaderNoMip( "menu/art/cursor" );
	uis.cursorCorona    = trap_R_RegisterShaderNoMip( "menu/art/cursor_corona" );
	uis.rb_on           = trap_R_RegisterShaderNoMip( "menu/art/switch_on" );
	uis.rb_off          = trap_R_RegisterShaderNoMip( "menu/art/switch_off" );

	uis.whiteShader = trap_R_RegisterShaderNoMip( "white" );

	uis.menuBackStd1 = trap_R_RegisterShaderNoMip( "menu/art/altback1" );
	uis.menuBackStd2 = trap_R_RegisterShaderNoMip( "menu/art/altback2" );
	uis.menuBackFog  = trap_R_RegisterShaderNoMip( "menu/art/fog" );
	uis.menuBackLandscape = trap_R_RegisterShaderNoMip( "menubacklandscape" );
